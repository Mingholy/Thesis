...
\advisor{\hspace{+4.0em}刘忠~教授~中国科学院成都计算机应用研究所}% supervisor
...
（3）基于长短期记忆网络与条件随机场构建了序列标注框架，在以文言文本为主的特定领域语料上进行症状术语识别实验。并在此基础上，针对语料特点，在不增加人工标注成本的同时，制订了额外的字符级别特征，提升了命名实体识别的效果。
...
(3) A sequence labeling frame was build based on long short-term and conditional random field. Experiments are implemented on ancient Chinese corpus. Furthermore, we developed character level feature without extra annotation cost according to the entity structure in the task, thus enhanced the performance of the sequence labeling frame above.
